...
class GVPConvLayer(nn.Module):
    ...
    :param vector_gate: whether to use vector gating.
                        (vector_act will be used as sigma^+ in vector gating if `True`)
    '''
    def __init__(self, node_dims, out_dims, edge_dims,
                 n_message=3, n_feedforward=2, drop_rate=.1,
                 autoregressive=False, 
                 activations=(F.relu, torch.sigmoid), vector_gate=False, norm_coors=False):
        ...
        GVP_ = functools.partial(GVP, 
                activations=activations, vector_gate=vector_gate)
        self.norm = nn.ModuleList([LayerNorm(node_dims, norm_coors=norm_coors) for _ in range(2)])
...
class GVPOrig_Decoder(Decoder):
    ...
    def forward(
        ...
        if len(self.linear) > 0:
            ...
            if self.use_edge_vectors:
                num_vectors = edge_vectors.shape[-2]
                edge = torch.cat([edge, rearrange(edge_vectors, "b l k n d -> b l k (n d)")], dim=-1)
            else:
                edge = h_E
            if isinstance(edge, tuple):
                h_E, edge_vectors = edge
