...
class LayerNorm(nn.Module):
    ...
    def __init__(self, dims, norm_coors=False):
...
class GVPConvLayer(nn.Module):
    ...
    def __init__(
        ...
        self.norm = nn.ModuleList(
            [LayerNorm(node_dims, norm_coors=norm_coors) for _ in range(2)]
        )
        ...
        if n_feedforward == 1:
            ...
            ff_func.append(GVP_(hid_dims, out_dims, activations=(None, None)))
...
class GVP_Encoder(Encoder):
        ...
        self.encoder = nn.ModuleList(
            [
                GVPConvLayer(
                    ...
                    n_message=args.num_encoder_mpnn_layers,
                    n_feedforward=2,
    ...
    def forward(
        ...
        if self.use_edge_vectors:
            ...
            edge = torch.cat(
                [h_E, rearrange(edge_vectors, "b l k n d -> b l k (n d)")], dim=-1
            )
        else:
            edge = h_E
        h, edge_attr, edge_index, vectors, batch, _ = to_pyg(
            h_V, edge, E_idx, mask, vectors
        )
        if self.use_edge_vectors:
            edge_vectors = edge_attr[..., -num_vectors * 3 :]
            edge_vectors = rearrange(edge_vectors, "l (n d) -> l n d", n=num_vectors)
            edge_attr = (edge_attr[..., : -num_vectors * 3], edge_vectors)
        x = (h, vectors)
...
class GVP_Decoder(Decoder):
    def __init__(self, args) -> None:
        ...
        self.decoder = nn.ModuleList(
            [
                GVPConvLayer(
                    ...
                    n_message=args.num_decoder_mpnn_layers,
                    n_feedforward=2,
                    drop_rate=args.dropout,
                    autoregressive=False,
                    activations=(F.relu, torch.sigmoid),
        ...
        linear = []
        for k in range(args.linear_layers_num):
            linear.append(
                GVPConvLayer(
                    node_dims[-1],
                    node_dims[-1],
    ...
    def forward(
        ...
        if len(self.linear) > 0:
            edge, indexes, *_ = self.edge_compute_func(
            ...
            if self.use_edge_vectors:
                num_vectors = edge_vectors.shape[-2]
                edge = torch.cat(
                    [edge, rearrange(edge_vectors, "b l k n d -> b l k (n d)")], dim=-1
                )
            else:
                edge = h_E
            if isinstance(edge, tuple):
            ...
            _, edge_attr, edge_index, *_ = to_pyg(h_V, edge, indexes, mask, None)
            if num_vectors is not None:
                edge_vectors = edge_attr[..., -num_vectors * 3 :]
                edge_vectors = rearrange(
                    edge_vectors, "l (n d) -> l n d", n=num_vectors
                )
                edge_attr = (edge_attr[..., : -num_vectors * 3], edge_vectors)
        ...
        h, vectors = x
        b, l, n = E_idx.shape
        h_V, _, upd = from_pyg(h, None, vectors, b, l, n)
        return (
            h_V,
            h_E,
            upd,
